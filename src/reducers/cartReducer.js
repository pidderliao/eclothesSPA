import { ADD_TO_CART, DELETE_FROM_CART, UPDATE_ITEM_UNITS, ADD_TO_DETAIL } from '../action/cartActions';
...
export default function cartReducer(state = [], action = {}) {
    switch (action.type) {
        ...
        case ADD_TO_DETAIL:
            let detailToDel = findProductIndex(state, action.payload.id);
            if (detailToDel !== -1) {
                return [...state.slice(0, detailToDel), ...state.slice(detailToDel + 1)];
            }
            return state.concat(action.payload);
        default:
            return state;
